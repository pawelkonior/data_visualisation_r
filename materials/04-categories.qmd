...
## Seaborn: wykresy kategoryczne

`stripplot()`, `countplot()` i `pointplot()` z `seaborn` mają bezpośrednie
odpowiedniki w gramatyce `ggplot2`: wykres punktowy z losowym rozsunięciem
punktów (strip plot), wykres zliczeń (count plot) i wykres punktów ze średnimi
(point plot).

```{r}
#| label: fig-school-stripplot
#| fig-cap: "Odpowiednik `stripplot()` z seaborn: każda kropka to jeden grant."
...
```{r}
#| label: fig-school-countplot
#| fig-cap: "Odpowiednik `countplot()` z seaborn: liczba szkół według modelu i regionu."
...
```{r}
#| label: fig-school-pointplot
#| fig-cap: "Odpowiednik `pointplot()` z seaborn: średnia kwota i przedział ufności."
