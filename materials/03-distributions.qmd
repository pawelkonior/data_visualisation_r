...
## Seaborn: histogram, gęstość i kreski obserwacji (rug plot)

Ten sam efekt, który w `seaborn` często robi `histplot(..., kde = TRUE)` albo
`displot(kind = "kde", rug = TRUE)`, w `ggplot2` powstaje przez dołożenie warstw:
histogramu, wykresu gęstości (density plot) i krótkich kresek pokazujących
pojedyncze obserwacje (rug plot).
...
## Seaborn: wykres łączony (`jointplot()`)

`jointplot()` łączy dwie perspektywy: relację między zmiennymi w środku oraz
rozkłady brzegowe na osiach. W R najczytelniej złożyć taki widok z panelu
głównego i dwóch małych rozkładów brzegowych.

```{r}
#| label: fig-fmr-jointplot-scatter
#| fig-width: 8.4
#| fig-height: 7.0
#| fig-cap: "Odpowiednik seabornowego `jointplot()` (joint plot): punkty, trend i histogramy marginalne. Skrajne wartości są przycięte dla czytelności."
...
```{r}
#| label: fig-fmr-jointplot-binned
#| fig-width: 8.4
#| fig-height: 7.0
#| fig-cap: "Odpowiednik `jointplot(kind = 'hist')`: liczność par wartości i gęstości marginalne. Jaśniejsze pola oznaczają więcej obserwacji."
